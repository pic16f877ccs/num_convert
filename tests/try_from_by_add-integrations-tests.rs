...
#[test]
fn from_out_i8() {
    assert_eq!(i8::MIN, <i8 as    TryFromByAdd>::from_i8(i8::MIN).unwrap());
    assert_eq!(i8::MIN, <i8 as    TryFromByAdd>::from_i8(i8::MIN).unwrap());
    assert_eq!(i8::MIN, <i8 as    TryFromByAdd>::from_u8(u8::MIN).unwrap());
    assert_eq!(i8::MAX, <i8 as    TryFromByAdd>::from_u8(u8::MAX).unwrap());
    assert_eq!(i8::MIN, <i8 as   TryFromByAdd>::from_u16(u8::MIN as u16).unwrap());
    assert_eq!(i8::MAX, <i8 as   TryFromByAdd>::from_u16(u8::MAX as u16).unwrap());
    assert_eq!(i8::MIN, <i8 as   TryFromByAdd>::from_u32(u8::MIN as u32).unwrap());
    assert_eq!(i8::MAX, <i8 as   TryFromByAdd>::from_u32(u8::MAX as u32).unwrap());
    assert_eq!(i8::MIN, <i8 as   TryFromByAdd>::from_u64(u8::MIN as u64).unwrap());
    assert_eq!(i8::MAX, <i8 as   TryFromByAdd>::from_u64(u8::MAX as u64).unwrap());
    assert_eq!(i8::MIN, <i8 as TryFromByAdd>::from_usize(u8::MIN as usize).unwrap());
    assert_eq!(i8::MAX, <i8 as TryFromByAdd>::from_usize(u8::MAX as usize).unwrap());
}

#[test]
fn from_out_u8() {
    assert_eq!(u8::MIN, <u8 as    TryFromByAdd>::from_i8(i8::MIN).unwrap());
    assert_eq!(u8::MIN, <u8 as    TryFromByAdd>::from_i8(i8::MIN).unwrap());
    assert_eq!(u8::MIN, <u8 as    TryFromByAdd>::from_u8(u8::MIN).unwrap());
    assert_eq!(u8::MAX, <u8 as    TryFromByAdd>::from_u8(u8::MAX).unwrap());
    assert_eq!(u8::MIN, <u8 as   TryFromByAdd>::from_u16(u8::MIN as u16).unwrap());
    assert_eq!(u8::MAX, <u8 as   TryFromByAdd>::from_u16(u8::MAX as u16).unwrap());
    assert_eq!(u8::MIN, <u8 as   TryFromByAdd>::from_u32(u8::MIN as u32).unwrap());
    assert_eq!(u8::MAX, <u8 as   TryFromByAdd>::from_u32(u8::MAX as u32).unwrap());
    assert_eq!(u8::MIN, <u8 as   TryFromByAdd>::from_u64(u8::MIN as u64).unwrap());
    assert_eq!(u8::MAX, <u8 as   TryFromByAdd>::from_u64(u8::MAX as u64).unwrap());
    assert_eq!(u8::MIN, <u8 as TryFromByAdd>::from_usize(u8::MIN as usize).unwrap());
    assert_eq!(u8::MAX, <u8 as TryFromByAdd>::from_usize(u8::MAX as usize).unwrap());
}
